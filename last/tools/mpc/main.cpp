...
KleeHandler::KleeHandler(int argc, char **argv) 
  ...
  if (OutputDir=="") {
	...
	char buf[256], tmp[64];
	sprintf(tmp, "klee-out-%s", Index.c_str());
	dirname = tmp;
	sprintf(buf, "%s/%s", directory.c_str(), tmp);
	theDir = buf;
  ...
  if (mkdir(m_outputDirectory, 0775) < 0) {
	...
	//string dir = m_outputDirectory;
	//std::string cmd = "mv -r " + m_outputDirectory + " " + m_outputDirectory + "_*";
	std::string cmd = "mv " + theDir + " " + theDir + "_*";
	system(cmd.c_str());

    assert(mkdir(m_outputDirectory, 0775) >= 0);
	//exit(1);
  }
