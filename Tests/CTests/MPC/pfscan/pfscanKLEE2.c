	     ...
	     int f)
{
    ...
    /*size_t localSize;
    klee_make_symbolic(&localSize, sizeof(localSize), "localSize");
    n_bytes += localSize;// sp->st_size;*/
    
    switch (f)
    {
      case FTW_F:
	pqueue_put(&pqb, (void *) __strdup(path));
	return 0;
     ...
     char *argv[])
{
  ...
  EndOptions:
    ...
    rstr = (unsigned char *) __strdup("pthread_t");
    //rstr = "pthread_t";
    rlen = deslash(rstr);
    printf("rlen: %s, %d\n", rstr, rlen);
