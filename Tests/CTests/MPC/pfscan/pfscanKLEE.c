...
int pqueue_put(PQUEUE *qp, void *item)
{
  ...
  if (qp->closed){
      printf("can't do put because qp is closed!\n"); 
      pthread_mutex_unlock(&qp->mtx);
      return 0;
  }
...
int pqueue_get(PQUEUE *qp, void **item)
{
  ...
  assert(!(qp->occupied <= 0 && !qp->closed));
...
int line_f  = 0;
int maxlen  = 64;
// ANDI this variable is not used sharedly, so I make it local
//int ignore_case = 0;
...
{
    printf("mmm: %s\n", path);
    //int code = ftw(path, foreach_path, max_depth);// yqp
    int code = foreach_path(path, NULL, FTW_F); //yqp
     ...
     char *argv[])
{
    ...
    klee_make_symbolic(&verbose, sizeof(verbose), "shared*verbose");
    verbose = 0;
    klee_make_symbolic(&debug, sizeof(debug), "shared*debug");
    debug = 0;
    klee_make_symbolic(&size, sizeof(size), "shared*pqb.qsize");
    pqb.qsize = size;
    klee_make_symbolic(&occupied, sizeof(occupied), "shared*pqb.occupied");
    pqb.occupied = occupied;
    klee_make_symbolic(&closed, sizeof(closed), "shared*pqb.closed");
    pqb.closed = closed;
    klee_make_symbolic(&n_matches, sizeof(n_matches), "shared*n_matches");
    n_matches = 0;
    // ANDI is it fine to fix this value?
    int ignore_case = 0;
    ...
    //CREST_shared_int(pqb.qsize);
    //CREST_shared_int(pqb.occupied);
    /*klee_make_symbolic(&size, sizeof(size), "shared*pqb.qsize");
    pqb.qsize = size;
    klee_make_symbolic(&occupied, sizeof(occupied), "shared*pqb.occupied");
    pqb.occupied = occupied;*/
    //CREST_shared_int(pqb.nextin);
    //CREST_shared_int(pqb.nextout);
    //CREST_shared_int(pqb.closed);
    //CREST_shared_int(n_matches);
    /*klee_make_symbolic(&closed, sizeof(closed), "shared*pqb.closed");
    pqb.closed = closed;
    klee_make_symbolic(&n_matches, sizeof(n_matches), "shared*n_matches");*/
    //mayb want to make pqb.buf a shared var as well?   
    
    
    //niloo 
    //I keep this but you don't use any option in the command line
  ...
  EndOptions:
    ...
    //rstr = (unsigned char *) strdup("pthread_t");
    rstr = (unsigned char *) ("pthread_t");
    rlen = deslash(rstr);
    printf("xxxx:%s, %d\n", rstr, rlen);
