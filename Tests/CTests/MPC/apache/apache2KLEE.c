...
int count, complete, refcount, cleanup;
...
int main(int argc, char **argv)
{
  ...
  klee_make_symbolic(&input3, sizeof(input3), "shared*input3");
  
  sh_obj = &__obj ; 

  sh_obj->count=1;   
  sh_obj->complete=input1;
  sh_obj->refcount=refcount=input2;
  ...
  *(sh_obj->key)='1';
  ...
  klee_make_symbolic(&count, sizeof(count), "shared*count");
  klee_make_symbolic(&complete, sizeof(complete), "shared*complete");
  klee_make_symbolic(&refcount, sizeof(refcount), "shared*refcount");
  klee_make_symbolic(&cleanup, sizeof(cleanup), "shared*cleanup");
  sh_obj->count = count;
  sh_obj->complete = complete;
  sh_obj->refcount = refcount;
  sh_obj->cleanup = cleanup;
