...
#define THRDMAX	3 //307	/* keep this prime - used in hashing fn */ 
...
char *trc_varargs(const char *fmt, ...)
{
	...
	HASH_READ_ENTER();
	if(t = trc_thread(1/*pthread_self()*/)){
		if(fmt){
			va_start(args, fmt);
...
void trc_trace(taction_t a, uint_t i, tlevel_t level, cchar_t *file, uint_t line, cchar_t *fn, cchar_t *kword, cchar_t *s)
{
	...
	if(!_fp || !(t = trc_thread(1/*pthread_self()*/))){
		HASH_READ_EXIT();
		return;
	}
...
int foo(void *arg)
{
    ...
    i++;
    TRC_PRINT(UNIT_FOO, TRC2, ("Hello world[%d]", i));
    i++;
    TRC_PRINT(UNIT_FOO, TRC1, ("Hello world[%d]", i));
    i++;
    TRC_PRINT(UNIT_FOO, TRC1, ("Hello world[%d]", i));
    ...
    TRC_ERROR(UNIT_FOO, ("Invalid argument1: %d", n));
    TRC_ERROR(UNIT_FOO, ("Invalid argument2: %d", n));
    TRC_ERROR(UNIT_FOO, ("Invalid argument3: %d", n));
...
int main(int argc, char **argv)
{
    ...
    klee_make_symbolic(_hashsem, sizeof(_hashsem), "shared*hashsem");
    klee_make_symbolic(_startclient, sizeof(_startclient), "shared*startclient");
