...
extern void trc_trace(taction_t a, uint_t i, tlevel_t tlevel, cchar_t *file,uint_t line, cchar_t *fn, cchar_t *kword, cchar_t *s, uint_t id);
...
/* ----------------------------------------------------------------------
   ...
   --------------------------------------------------------------------*/
#define TRC_ENTER(a,b,c, d) \
	if(_trc) \
		trc_trace(TRCA_ENTER, a, b, __FILE__, __LINE__, __FUNCTION__, NULL, trc_varargs c, d)
...
#define TRC_RETURN(a,b,c,d){ \
	if(_trc) \
		trc_trace(TRCA_RETURN, a, b, __FILE__, __LINE__, __FUNCTION__, NULL, trc_varargs c, d); \
	pthread_exit(NULL); \
   	return(d); \
}
...
/* ----------------------------------------------------------------------
   ...
   --------------------------------------------------------------------*/
#define TRC_VOID_RETURN(a,b, d){ \
	if(_trc) \
		trc_trace(TRCA_VOID_RETURN, a, b, __FILE__, __LINE__, __FUNCTION__, NULL, NULL, d); \
	return; \
}
...
/* ----------------------------------------------------------------------
   ...
   --------------------------------------------------------------------*/
/*#define TRC_PRINT(a,b,c, d) \
	if(_trc) {\
		trc_trace(TRCA_PRINT, a, b, __FILE__, __LINE__, __FUNCTION__, NULL,trc_varargs c, d); \
	} \
	else { \
	    assert(0);\
	}*/
...
/* ----------------------------------------------------------------------
   ...
   --------------------------------------------------------------------*/
#define TRC_ERROR(a,b, d) \
	if(_trc) \
		trc_trace(TRCA_ERROR, a, TRC_ERR, __FILE__, __LINE__, __FUNCTION__, NULL, trc_varargs b, d)
...
int trc_init(cchar_t *file, tenable_t trc, ton_t on, tlevel_t level, uint_t umax, tserver_t server)
{
	...
	if(umax > 0){
		_unitmax = umax;
		_unit = (tunit_t *)malloc(sizeof(tunit_t)*_unitmax); klee_make_symbolic(_unit, sizeof(*_unit), "shared*unit");
...
/* ----------------------------------------------------------------------
   tthread_t *		// thread struct of shared data   
...
{
    fprintf(stderr, "trc_add_thread\n");
  
	uint_t i;
	tthread_t *t = NULL;

	fprintf(stderr, "tat.a\n");
	
	if(id == 0) {
		id = pthread_self();
	}
	
	fprintf(stderr, "tat.b\n");

	HASH_WRITE_ENTER();
	
	fprintf(stderr, "tat.c\n"); 
    
    /*if (tname == "foo")
        klee_make_symbolic(&t, sizeof(t), "shared*foot");
    else
        klee_make_symbolic(&t, sizeof(t), "shared*bart");*/
	
	if(t = trc_thread(id)){
	    fprintf(stderr, "tat.d\n");
...
/* ----------------------------------------------------------------------
   ...
   --------------------------------------------------------------------*/
void trc_trace(taction_t a, uint_t i, tlevel_t level, cchar_t *file, uint_t line, cchar_t *fn, cchar_t *kword, cchar_t *s, uint_t id)
{
	fprintf(stderr, "trc_trace.a)\n");
	...
	fprintf(stderr, "initially: n = %d\n", n);
	
	fprintf(stderr, "trc_trace.b)\n");

	HASH_READ_ENTER();
	
	fprintf(stderr, "trc_trace.c)\n");
	
	/* check for valid output stream and this thread is registered */
	if(!_fp || !(t = trc_thread(id/*pthread_self()*/))){
		HASH_READ_EXIT();
		return;
	}
	
	fprintf(stderr, "trc_trace.d)\n");
	...
	fprintf(stderr, "trc_trace.e)\n");

	time(&tim);
	timeptr = localtime(&tim);
	strftime(buf, 64, "%D-%T", timeptr);
	
	fprintf(stderr, "trc_trace.f.1)\n");

	/* generate trace output */
	fprintf(stderr, "n = %d -- before\n", n);
	//sprintf(t->trace, "%s:%s:%d:%s:%n", buf, file, line, t->name, &n);
	sprintf(t->trace, "%s:%s:%d:%s:%n", buf, "ctrace1.c", line, t->name, &n);
	//sprintf(t->trace, "%s:%s:%d:%n:%s", file, line, t->name, &n, buf);
	fprintf(stderr, "%s\n", t->trace);
	fprintf(stderr, "trc_trace.f.2)\n");
	fprintf(stderr, "n = %d\n", n);
	fprintf(stderr, "WSPACELEN(%d) = %d\n", n, WSPACELEN(n));
	/*if (WSPACELEN(n) < 0) {
	  fprintf(stderr, "Bad behaviour!\n");
	  assert(0);
	}*/
	assert((WSPACELEN(n)) >= 0);
	t->wspace[WSPACELEN(n)] = '\0';	
	
	fprintf(stderr, "trc_trace.g)\n");

	if(a == TRCA_ENTER) ;//sprintf(t->trace+n, "%senter %s(%s)\n", t->wspace, fn, s);
	else {if(a == TRCA_RETURN) ;//sprintf(t->trace+n, "%sreturn %s(%s)\n", t->wspace, fn, s);
	else {if(a == TRCA_VOID_RETURN) ;//sprintf(t->trace+n, "%s%s: return\n", t->wspace, fn);
	else {if(a == TRCA_PRINT) ;//sprintf(t->trace+n, "%s%s\n", t->wspace, s);
	else {if(a == TRCA_ERROR)	;//sprintf(t->trace+n, "%sERROR in fn %s: %s\n", t->wspace, fn, s);
    }}}}
    
	fprintf(stderr, "trc_trace.h)\n");
	...
	fprintf(stderr, "trc_trace.i)\n");
	
	HASH_READ_EXIT();
	
	fprintf(stderr, "trc_trace.j)\n");
}
...
int foo(void *arg)
{
    ...
    TRC_ADD_THREAD("foo", 1); // 0
    
    fprintf(stderr, "foo.b1\n");
    
    TRC_ENTER(UNIT_FOO, TRC1, ("n=%d", n), 1);
		
    fprintf(stderr, "foo.c\n");
    
    if(n <= 0){
        TRC_ERROR(UNIT_FOO, ("Invalid argument: %d", n), 1);
    ...
    for(i=n; i>0; i--){
	    //sleep(1);
        TRC_PRINT(UNIT_FOO, TRC1, ("Hello world[%d]", i), 1);
    }
...
void *bar(void *arg)
{
    ...
    TRC_ADD_THREAD("bar", 1); // 0
    //TRC_ADD_THREAD("bar", 1);
    
    fprintf(stderr, "bar.b\n");

    TRC_END();
    
    fprintf(stderr, "bar.c\n");
...
int main(int argc, char **argv)
{
    ...
    //niloo
    // make <num_msgs> a havoc variable 
    ...
    /*CREST_shared_int(_trc);
    CREST_shared_int(_on);
    CREST_shared_int(_tlevel);
    CREST_shared_int(_server);*////////////////////
	klee_make_symbolic(&_trc, sizeof(_trc), "shared*trc");
	klee_make_symbolic(&_on, sizeof(_on), "shared*on");
	klee_make_symbolic(&_tlevel, sizeof(_tlevel), "shared*tlevel");
	klee_make_symbolic(&_server, sizeof(_server), "shared*server");
    klee_make_symbolic(_thread, sizeof(_thread), "shared*thread");

    _trc = TRC_DISABLED;		/* trace enabled ? */
    _on = TRC_OFF;		/* trace on */	
    _tlevel = TRC_NONE;	/* trace levels enabled */
    _server = 0;		/* run trace library as separate thread */

    klee_make_symbolic(&_startclient, sizeof(sem_t), "shared*_startclient");
    klee_make_symbolic(&_hashsem, sizeof(sem_t), "shared*_hashsem");
    klee_make_symbolic(&_msgs, sizeof(_msgs), "shared*msgs");
	klee_make_symbolic(&_serv_sockfd, sizeof(_serv_sockfd), "shared*servsockfd");
	klee_make_symbolic(&_initialised, sizeof(_initialised), "shared*initialised");
    _initialised = 0;		/* static trace library initialised */

    _msgs = 0;			/* msgs still being processed by server */
    
    //niloo: make it have server
    TRC_INIT(NULL, TRC_ENABLED, TRC_ON, TRC_ALL, UNIT_MAX, 0);
    //TRC_INIT(NULL, TRC_ENABLED, TRC_ON, TRC_ALL, UNIT_MAX, 1);
    ...
    //n = n % 5;
    n = 1;
    fprintf(stderr, "param n = %d\n", n);
    //n = atoi(argv[2]);
    pthread_create(&tid1, NULL, foo, (void*)n);
    pthread_create(&tid2, NULL, bar, (void*)n);
