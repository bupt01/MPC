...
int _trc;		/* trace enabled ? */
ton_t _on = TRC_OFF;		/* trace on */	
tlevel_t _tlevel = TRC_NONE;	/* trace levels enabled */
FILE *_fp = NULL;		/* trc msgs output file stream */
tserver_t  _server = 0;		/* run trace library as separate thread */
int _msgs = 0;			/* msgs still being processed by server */
pthread_t _sid;			/* server thread id */
int _serv_sockfd;		/* server socket */
sem_t _startclient;		/* dont start client until server inited */
int _initialised = 0;		/* static trace library initialised */
...
#define THRDMAX	        3	/* keep this prime - used in hashing fn */ 
...
int trc_init(cchar_t *file, tenable_t trc, ton_t on, tlevel_t level, uint_t umax, tserver_t server)
{
	...
	if(pthread_mutex_init(&_hashmutex, NULL)){
		printf("return after mutex_init\n");	
   		return 1;
    }
...
void *bar(void *arg)
{
  fprintf(stdout, "bar...\n");
    ...
    //here to have a seq test of bar
    //int input;
    //__fusion_havoc_int(&input);
    //if(input>0)
    //    printf("input >0 \n");
    //else
    //    printf("NOT input >0 \n");    
    ...
    */
    ...
    //TRC_PRINT(UNIT_BAR, TRC1, ("0x%x", NULL));
    TRC_END();
...
int main(int argc, char **argv)
{
    ...
    //n = n % 5;
    n = 1;
    fprintf(stdout, "param n = %d\n", n);
    //n = atoi(argv[2]);
    pthread_create(&tid1, NULL, foo, &n);
    pthread_create(&tid2, NULL, bar, &n);
