...
void * thread_routine(int* arg)
{
      ...
      mode = cp_splaytree_get_mode(t);

      char *x;
      char name[80];
      int tmp;

      sprintf(name, "ehhghgfcg");
      name[strlen (name) - 1] = '\0';	/* chop newline */

                
      cp_splaytree_set_mode(t, mode);
...
main ()
{
  ...
  t = cp_splaytree_create_by_option(COLLECTION_MODE_COPY,
