...
namespace scheduleLIB{
    ...
    void saveScheduleFile2(std::string filename, const std::vector<std::string>& listOperation, std::map<std::string, std::pair<int, std::string> > prefix);  //yqp: save a given operation's vector and store it in a given filename
    ...
    std::vector<std::string> schedule2string2(const Schedule& schedule); // yqp: transform a given schedule do a string's vector
