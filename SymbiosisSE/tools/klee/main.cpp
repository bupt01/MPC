...
/***/
std::string Times, Index, Last, Solutions, Parallel, Trace;
...
private:
  ...
  std::unique_ptr<llvm::raw_ostream> m_traceFile;
...
public:
  ...
  llvm::raw_ostream &getTraceStream() const { return *m_traceFile; }
  ...
  if (dir_given) {
    ...
    if (mkdir(d.c_str(), 0775) == 0) {
      ...
      if (((unlink(klee_last.c_str()) < 0) && (errno != ENOENT)) ||
       ...
       klee_error("cannot create \"%s\": %s", d.c_str(), strerror(errno));
    }

    std::cerr << "KLEE: output directory = \"" << dirname << " " << Index << " " << Last << "\"\n";
  }
...
static void parseArguments(int argc, char **argv) {
  ...
  for (int i=1; i<argc; i++) {
    ...
    if((s.find("--times")<s.size())||(s.find("--index")<s.size())||(s.find("--last")<s.size())||(s.find("--trace")<s.size())){
       arguments.pop_back();
    }
  }
...
void parse_args(int argc, char *const* argv) {
	...
	while (1) {
		static struct option long_options[] = {
			{"allow-external-sym-calls", no_argument, 0, 'a'},
			{"sym-args", required_argument, 0, 'r'},
			{"bb-trace", required_argument, 0, 'b'},
			{"times", required_argument, 0, 't'},
			{"index", required_argument, 0, 'i'},
			{"last", required_argument, 0, 'l'},
			{"libc", required_argument, 0, 'c'},
			{"solutions", required_argument, 0, 's'},
			{"posix-runtime", no_argument, 0, 'p'},
			{"parallel", required_argument, 0, 'e'},
			{"trace", required_argument, 0, 'x'},
			{"", }
		};
...
int main(int argc, char **argv, char **envp) {
  ...
  std::cout<<"replayOutDir:"<<ReplayKTestFile.ValueStr.str()<<std::endl;
  if (!ReplayKTestDir.empty() || !ReplayKTestFile.empty()) {
    ...
    std::cout<<"seeds'size is :"<<seeds.size()<<std::endl; //ing
    for (std::vector<std::string>::iterator
    ...
    interpreter->runFunctionAsMain(mainFn, pArgc, pArgv, pEnvp);//走到这了
  ...
  if (Parallel == "1"/*parallel*/) {
	  pid_t curPID = getpid();
	  std::string cmd = "$INSTALL_PATH/symbiosis-master/SymbiosisSolver/symbiosisSolver --trace-folder="+Trace+"/klee-out-"+Index+" --model=$INSTALL_PATH/symbiosis-master/SymbiosisSolver/tmp/modelCrasher.txt --solution=$INSTALL_PATH/symbiosis-master/SymbiosisSolver/tmp/failCrasher.txt --with-solver=$INSTALL_PATH/z3/build/z3 --parallel=1 --times=" + Times + " --last=" + Index + " --solutions=" + Solutions + " --trace=" + Trace + " --bcfile="+bcFile;
	  system(cmd.c_str());
  }
  ...
  if (Parallel == "1") {
	  ...
	  int num = intValue(str);

	  std::ofstream outFile;
	  outFile.open("paralleled", std::ios::app);
	  if (!outFile.is_open()) {
		  std::cerr << "-> Error opening file paralleled" << ".\n";
		  outFile.close(); assert(0);
		  exit(1);
	  }
