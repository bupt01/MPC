...
//std::pair<std::unique_ptr<PTreeNode> , std::unique_ptr<PTreeNode>>
PTree::split(Node *n, 
  ...
  // n->left = std::make_unique<Node>(n,leftData);//new Node(n, leftData);
  // n->right = std::make_unique<Node>(n,rightData);//new Node(n, rightData);
  // return std::make_pair(n->left, n->right);
  return std::make_pair(new Node(n, leftData), new Node(n, rightData));
}
