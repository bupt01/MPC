...
bool ConstraintManager::rewriteConstraints1(ExprVisitor &visitor) {
  ...
  std::vector<std::pair<int, int> > oldIndexes;
  oldIndexes.insert(oldIndexes.begin(), indexes.begin(), indexes.end());
  indexes.clear();
  constraints1.swap(old);
  for (ConstraintManager::constraints_ty::iterator 
         it = old.begin(), ie = old.end(); it != ie; ++it) {
    ref<Expr> &ce = *it;
    ref<Expr> e = visitor.visit(ce);
    std::pair<int, int> index = oldIndexes[it-old.begin()];

    if (e!=ce) {
      addConstraintInternal1(e, index); // enable further reductions
      changed = true;
    } else {
		constraints1.push_back(ce);
...
void ConstraintManager::addConstraintInternal1(ref<Expr> e, std::pair<int, int> index) {
  ...
  case Expr::Constant:
    assert(cast<ConstantExpr>(e)->isTrue() && 
           "attempt to add invalid (false) constraint");
    //constraints1.push_back(e);
    break;
    
    // split to enable finer grained independence and other optimizations
  case Expr::And: {
    BinaryExpr *be = cast<BinaryExpr>(e);
    addConstraintInternal1(be->left, index);
    addConstraintInternal1(be->right, index);
    break;
  }
  ...
  case Expr::Eq: {
    ...
    if (isa<ConstantExpr>(be->left)) {
      ExprReplaceVisitor visitor(be->right, be->left);
      bool f = rewriteConstraints1(visitor);
	}
    if (prefixNum.size() == 0 || prefixNum[0] == index.second) {
        constraints1.push_back(e);
        indexes.push_back(index);
        if (prefixNum.size() != 0)
            prefixNum.erase(prefixNum.begin());
    }
    break;
  }
...
void ConstraintManager::addConstraint1(ref<Expr> e, std::pair<int, int> index) {
  ////////////////////
  e = simplifyExpr1(e);
  addConstraintInternal1(e, index);
